...
import { createProbeSession } from "./probe/api";
import { LiveKitProbeTransport } from "./probe/livekitTransport";
import { createAttemptIdentifiers, type ProbeStatusPacket } from "./probe/protocol";
import { initialProbeState, reduceProbeState } from "./probe/state";
...
export default function App() {
  ...
  const canStart = !["connecting", "recording", "transferring", "replaying"].includes(
    state.phase,
  );
  ...
  return (
    <main className="probe-shell">
      <section className="probe-card" aria-live="polite">
        ...
        <p className="lede">
          Record a short phrase, stop, then hear the Python participant replay it once through
          LiveKit. Use headphones.
        </p>
        ...
        {state.metrics ? (
          <dl className="metrics">
            <div>
              <dt>Frames</dt>
              <dd>{state.metrics.frameCount}</dd>
            </div>
            <div>
              <dt>Captured audio</dt>
              <dd>{(state.metrics.audioDurationMs / 1000).toFixed(2)} s</dd>
            </div>
          </dl>
        ) : null}
